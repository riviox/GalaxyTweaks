...
class Frame(ctk.CTkFrame):
    def __init__(self, master, **kwargs):
        ...
        self.label = ctk.CTkLabel(self, text="GalaxyTweaks")
...
class App(ctk.CTk):
    ...
    def dwmt():
        ...
        """)
    tweaks_buttons = [
        ("Tweaks", lambda: self.handle_exception(tw)),
        ("DWM Tweaks", lambda: self.handle_exception(dwmt)),
        ("Internet Tweaks", lambda: self.handle_exception(netw)),
        ("Cleaner", lambda: self.handle_exception(cleaner)),
        ("Additional Tweaks", lambda: self.handle_exception(adwt)),
        ("About", lambda: self.handle_exception(about)),
        ("Registry Backup", lambda: self.handle_exception(bckp)),
    ]

    for i, (text, command) in enumerate(tweaks_buttons):
        button = ctk.CTkButton(self.frame, text=text, command=command)
        button.grid(row=i + 1, column=0, pady=5, sticky="nsew")
